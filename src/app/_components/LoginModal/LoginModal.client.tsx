...
const LoginModal = () => {
  ...
  // before선 스타일
  const beforeLine =
    "before:content-[''] before:block before:w-full before:h-[0.5px] before:bg-[#66666671] before:absolute before:top-1/2 before:left-1/2 before:-translate-x-1/2 before:-translate-y-1/2";
    ...
    >
      <div className="relative" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <div
            aria-label="로그인 설명"
            className="p-[9px_12px] text-[11px] text-center mt-[8px]"
          >
          ...
          <div
            className={`text-[13px] w-full text-center relative h-[24px] mt-[6px] ${beforeLine}`}
          >
          ...
          <ol
            aria-label="소셜 로그인 모음"
            className="mt-[16px] flex flex-col gap-[12px]"
          >
            <li aria-label="카카오 로그인">
              <button
                onClick={onClickKakaoButton}
                className="w-[95%] text-[14px] mx-auto p-[10px_12px] border-[1px] rounded-[6px] text-[#343434e5] border-solid border-[#6666662d] flex items-center shadow-sm bg-white hover:brightness-[90%]"
              >
                <Image
                  alt="로그인 버튼"
                  width={20}
                  height={20}
                  src="/loginModal/kakao.webp"
                  className="rounded-full mr-[8px]"
                />
                <span className="font-[600]">카카오 간편 로그인</span>
              </button>
            </li>
          </ol>
          ...
          <button
            onClick={toggle}
            className="absolute scale-75 top-0 right-0 m-[12px_12px_0_0] bg-white hover:brightness-90"
          >
